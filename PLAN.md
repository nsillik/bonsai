...
Status: **Phases 0, 1 and 2 landed** — PRs open on all three forks, CI for Phase 0/1 green on all three
platforms. Phase 3 is next, and is now **smaller** than originally planned: Phase 2 had to pull its
minimum forward. See [Next session](#next-session) to pick up, [Progress](#progress) for branches,
SHAs and evidence, and [Deviations](#deviations-from-this-plan) for what implementation turned out to
...
**Phase 3 — renderer runs.** Most of Phase 3's items 1–5, 7 and 8 landed in Phase 2, because the
window could not open without them (Deviations #13). Read the
[Phase 3](#phase-3--renderer-runs) section and its *Already landed in Phase 2* list before starting,
and do not redo them.

What is actually left of Phase 3:

1. Item 6 — the `world_edit` SSBO→TBO conversion is done, so what remains is confirming the terrain
   brush actually edits terrain on screen. Nothing has exercised that path yet: `terrain_gen` has not
   been run, and the world-edit path needs a brush applied to a chunk.
2. Item 9 — `SetVSync`, still dead. Decide wire-it-up-for-both vs delete.
3. The gate itself: `terrain_gen` and `blank_project` rendering and matching Linux.
...
2. **Build on Linux before pushing anything that touches a shared header.** `uintptr_t` compiled
...
3. **A GLSL version bump is a hard stop, not a warning.** `CheckShaderCompilationStatus` calls
   `Error`, which traps the process. So any shader that fails to compile takes the engine down at
   startup rather than degrading.
4. **Never write `Jesse` in a comment you author** — that is the upstream maintainer. Use
   `NOTE(nsillik)` / `TODO(nsillik)`. See `AGENTS.md`.
5. **Never push or PR to `scallyw4g/*`.** Verify with
   `gh pr list -R scallyw4g/bonsai --author=@me --state=open`.
6. **Do not run `gh stack`** — it resolves to the parent repo and will try to open upstream PRs.
7. **Pass `-R nsillik/bonsai`** to `gh` commands; `branch.master.remote` points at `upstream`.
8. macOS builds are x86_64 and run under Rosetta 2. `ARCH` is the **target** arch, not the host arch.
9. **Driving the machine to verify input is a user-visible act.** Do not post synthetic events to
...
graph LR
  ...
  P2 --> P3["Phase 3<br/>renderer runs<br/><i>items 1-5,7,8 landed in P2</i>"]
...
- ✅ CLI-launched GUI: `setActivationPolicy:NSApplicationActivationPolicyRegular` +
...
## Phase 3 — renderer runs

**MOSTLY LANDED IN PHASE 2.** Items 1, 2, 3, 4, 5, 7 and 8 are done — they had to be, because the
window cannot open on a 4.1 core context without them (Deviations #13). Item 6's conversion is done
but unexercised. Item 9 is still dead code.
...
| 4 | `glMultiDrawArraysIndirect` → loop of `glDrawArraysIndirect` | `render.cpp` `MultiDrawIndirect` |
...
| # | Change | Note |
|---|---|---|
| 6 | Confirm the world-edit path actually edits terrain | Blocked on regenerating `brushes/` (Deviations #18), not on the port |
| 9 | `SetVSync` | Still never called. Wire it up for both platforms or delete it |
| — | The gate itself | `terrain_gen` and `blank_project` rendering, visually matching Linux |

Two corrections to the original table, both measured:

- **Item 2 said "the 2 remaining SSBO shaders; the other 54 already compile at 410".** True of
  *compilation*, and it hid the real problem: `composite.fragmentshader` compiled at 410 all along and
  then **segfaulted the linker** on an empty `case` body. See Deviations #14. A compile-only audit is
  not sufficient evidence that a shader works on this driver.
- **Item 9 is dead code as written**, confirmed again in Phase 2: `SetVSync` has no live callers.

### Gate

Launch `terrain_gen` and `blank_project`; confirm rendering matches Linux (visual). Profile the
indirect-loop fallback against `MultiDrawArraysIndirect` (`render.cpp:1746` is a single call today;
the fallback becomes `DrawCommandsAt` calls plus `DrawCommandsAt` uniform uploads). Confirm shader
...
Blocker 1. Do not let it block Phase 3's gate.
...
| Items 3, 4 — `gl_DrawID` → per-draw uniform | `gl_DrawIndex` via `VK_KHR_shader_draw_parameters` — **1:1 with the existing `gl_DrawID`**, no engine change |
...
| Item 9 — `SetVSync` | `VK_PRESENT_MODE_FIFO_KHR` |
